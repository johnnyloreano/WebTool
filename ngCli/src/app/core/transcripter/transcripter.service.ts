...
export class TranscripterService {
  ...
  let deltaX = this.getDelta(actualAmino[0] , predecessorAmino[0]);
  let deltaY = this.getDelta(actualAmino[1] , predecessorAmino[1]);
  ...
  getTransition(actualAmino:number[], predecessorAmino:number[]) : string[]{
    let degrees = this.getDegrees(actualAmino, predecessorAmino);
    let quadrants =  this.quadrantObject(actualAmino,predecessorAmino);
    let calculateDegrees = this.getCorrectDegree(actualAmino,predecessorAmino,quadrants,degrees);
    let hours = [ this.toHour(calculateDegrees['predecessor']), 
                  this.toHour(calculateDegrees['actual']) ];
    console.log(calculateDegrees)
    return [ this.createText(hours[0][0],hours[0][1]), this.createText(hours[1][0], hours[1][1]) ].reverse();
  }
}
