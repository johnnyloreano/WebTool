...
export class ProteinViewerComponent implements OnInit {
  ...
  loadComponent() {
    ...
    for (let i = 0; i < arrLabel.length; i++) {
        ...
        if (i > 0) {
          // Sound placement
          const transitions = this._transcripter.getTransition(arrComponent[i].position, arrComponent[i - 1].position);
        ...
        if (i === arrLabel.length - 1) {
          ...
          arrComponent[i]._isFirst = true; arrComponent[i].downSound = 'Você saiu da proteína!';
        }
    }
  }
