...
export class AminoModal extends DialogComponent<AminoModel, boolean> implements AminoModel, OnInit {
  ...
  constructor(private _componentFactoryResolver: ComponentFactoryResolver,
     private _dialogService: DialogService, private _HttpRequester: HttpService, private _dataService: DataService) {
  ...
  insertComponent() {
    const componentFactory = this._componentFactoryResolver.resolveComponentFactory(LabelComponent);
  ...
  async instantiateAmino(aminoName) {
    const content = await this._HttpRequester.requestAmino(aminoName);
    const amino = new Aminoacid();
